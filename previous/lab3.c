...
struct interrupt_data
{
	...
	volatile int* rooms_addr;
	volatile int* dir_addr;
	volatile int* flag_addr;
};
...
static void handle_sliders_interrupt(struct interrupt_data* data)
{
    int sliders = IORD(SW_SLIDERS_BASE, 0);
    volatile int* rooms = data->rooms_addr;
    //int* rooms = data->rooms_addr;
    int leds = *(data->leds_addr);
    int flag = *(data->flag_addr);
    int dir = *(data->dir_addr);
