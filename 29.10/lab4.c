...
static void handle_sliders_interrupt(struct interrupt_data *data)
{
	...
	if(rooms[S1] > 0) {
    	...
    	S1on = alt_timestamp();
		 ...
		 S1off = alt_timestamp();
		 if(count_period_sec(S1off, S1on) > SEC_TIMEOUT) {
			 leds &= ~(LED_S1);
		 }
	 }
    ...
    if(rooms[S2] > 0) {
    	...
    	S2on = alt_timestamp();
		 ...
		 S2off = alt_timestamp();
		 if(count_period_sec(S2off, S2on) > SEC_TIMEOUT) {
			 leds &= ~(LED_S2);
		 }
    }
    ...
    if(rooms[S3] > 0) {
    	...
    	S3on = alt_timestamp();
		 ...
		 S3off = alt_timestamp();
		 if(count_period_sec(S3off, S3on) > SEC_TIMEOUT) {
			 leds &= ~(LED_S3);
		 }
    }
    ...
    if(rooms[S4] > 0) {
    	...
    	S4off = alt_timestamp();
    	if(count_period_sec(S4off, S4on) > SEC_TIMEOUT) {
    		leds &= ~(LED_S4);
    	}
    }
    ...
    if(rooms[S5] > 0) {
    	...
    	S5off = alt_timestamp();
    	if(count_period_sec(S5off, S5on) > SEC_TIMEOUT) {
    		leds &= ~(LED_S5);
    	}
    }
